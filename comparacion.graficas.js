...
function mostrarValoresEnModal(datos) {
    ...
    for (let i = 0; i < datos['Edad Gestacional'].length; i++) {
        ...
        pfe = (datos['PFE'].length > 0) ? (('string' == typeof datos['PFE'][i][0]) ? datos['PFE'][i][0] : Number(datos['PFE'][i][0]).toFixed(0)) : 0
        pfePCT = (datos['PFE'].length > 0) ? (('string' == typeof datos['PFE'][i][1]) ? datos['PFE'][i][1] : Number(datos['PFE'][i][1]).toFixed(0)) : 0
...
function informeComparacion()
{
    ...
    if (_hchartsDos.hasOwnProperty('axes') !== false){ _hchartsDos.setSize(430, 250, false); _hchartsDos.reflow(); }
    if (_hchartsTres.hasOwnProperty('axes') !== false){ _hchartsTres.setSize(430, 250, false); _hchartsTres.reflow(); }
    ...
    _informe.innerHTML += '<span class="mt-2" style="border-top:1px solid #000;width:100%!important;display:block;padding-top:2px"></span><div class="row"><div class="col-6"><p style="font-size: 15px"><strong>Identificación de paciente: </strong><span id="paciente"></span></p></div><div class="col-6"><p style="font-size: 15px" class="text-right"><strong>DNI: </strong><span id="rut"></span></p></div></div><div class="row"><div class="col-12"><div id="graficoUno"></div></div><div class="col-12"><div id="graficoDos" class="row"></div></div></div><h5 class="text-center" style="padding-top:2px">Tabla percentiles de PFE más indice de pulsatilidad (IP) para Doppler de Uterinas, UMB, ACM, CCP.</h5><span class="my-2" style="border-top:1px solid #000;width:100%!important;display:block;padding-top:2px"></span><div class="row" id="lineclear"><div class="col-12" id="tablaDatos"></div></div>'
    _informe.innerHTML += '<hr><div class="row mt-4"><div class="col-8"><p style="font-size: 15px"><strong>Fecha de Impresion: </strong><span id="fechaImpresion"></span></p></div><div class="col-4"><p style="font-size: 15px"><strong>Ecografista: </strong><span id="examinador"></span></p></div></div>';
    _informe.innerHTML += '<span style="border-top: 2px solid #000;width: 100% !important;display: block;"></span><p style="margin-bottom:0;" class="pie-pagina">Peso Fetal categorizado por curva fetal de la OMS https://www.scielo.cl/scielo.php?script=sci_arttext&pid=S0717-75262023000300183<br>Doppler Uterinas: Gómes O., Figueras F., Fernandez S., Bennasar M., Martinez JM., Puerto B., Gratacos E., UOG 2008; 32: 128c - 32. Doppler Fetal: Baschat AA, Gembruch U, Ultrasound Obstet. Ginecol. 2003; 21 : 124 - 127</p><p style="margin-bottom:0 !important;"><strong>El software tiene por objetivo favorecer análisis preliminar de datos obtenidos en exámen ecográfico, la interpretación clínica de los mismos, inicialmente es responsabilidad exclusiva de quien realiza y certifica este documento.</strong> </p>'
